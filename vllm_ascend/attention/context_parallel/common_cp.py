...
def _npu_update_dycp_attn(num_dycp_reqs, attn_output: torch.Tensor, softmax_lse: torch.Tensor) -> torch.Tensor:
    logger.info(
        f"chenxiao--debug dycp_attn_update_enter num_dycp_reqs={num_dycp_reqs}, "
        f"attn_output_shape={tuple(attn_output.shape)}, softmax_lse_shape={tuple(softmax_lse.shape)}"
    )
    ...
    logger.info(
        f"chenxiao--debug dycp_attn_update_exit output_type={type(attn_out).__name__}, "
        f"output_shape={tuple(attn_out.shape)}"
    )
    return attn_out
