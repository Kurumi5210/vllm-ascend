...
class AscendMlaCPImpl(AscendMLAImpl):
    ...
    def _forward_decode(
        ...
        logger.info(
            f"chenxiao--debug mla_decode_forward graph_key={graph_key}, capturing={forward_context.capturing}, "
            f"num_tokens={num_tokens}, num_dycp_reqs={num_dycp_reqs}, input_layout={input_layout}, "
            f"seq_lens_type={type(seq_lens).__name__}"
        )
        ...
        if self.dycp_size > 1 and num_dycp_reqs > 0:
            ...
            logger.info(
                f"chenxiao--debug mla_decode_dycp_update result_type={type(dycp_attn_output).__name__}"
            )
            if isinstance(dycp_attn_output, tuple):
                tuple_types = [type(x).__name__ for x in dycp_attn_output]
                logger.info(
                    f"chenxiao--debug mla_decode_dycp_update tuple_result_types={tuple_types}, "
                    f"tuple_len={len(dycp_attn_output)}"
                )
                if len(dycp_attn_output) == 0 or not isinstance(dycp_attn_output[0], torch.Tensor):
                    raise TypeError(
                        f"_npu_update_dycp_attn returned tuple without tensor head: {tuple_types}"
                    )
                dycp_attn_output = dycp_attn_output[0]
            if not isinstance(dycp_attn_output, torch.Tensor):
                raise TypeError(
                    f"_npu_update_dycp_attn returned unsupported type: {type(dycp_attn_output).__name__}"
                )
            logger.info(
                f"chenxiao--debug mla_decode_dycp_update assign_shape={tuple(dycp_attn_output.shape)}, "
                f"assign_dtype={dycp_attn_output.dtype}"
            )
            attn_output[:num_dycp_reqs] = dycp_attn_output.to(dtype=attn_output.dtype, device=attn_output.device)
